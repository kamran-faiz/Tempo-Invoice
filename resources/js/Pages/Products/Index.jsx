...
const Index = ({ products }) => {
  ...
  return (
    <AuthenticatedLayout title="Products List">
      <section className="p-margin_desktop space-y-stack_lg max-w-container_max_width mt-6">
        
        {/* */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-baseline gap-3">
            <h3 className="font-headline-md text-headline-md text-on-surface">All Products</h3>
            <span className="px-2 py-0.5 bg-surface-container text-on-surface-variant font-label-sm rounded-full">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Click configuration sets target item to null so form opens blank for fresh entries */}
            <button 
              onClick={() => { setSelectedProduct(null); setShowModal(true); }} 
              className="flex items-center gap-2 bg-primary-container text-on-primary-container px-5 py-2.5 rounded-xl font-label-md hover:opacity-90 transition-opacity"
            >
      ...
      {/* FIX 3: Dynamic state tracking dependencies linked down to parameters */}
      <ProductModal 
