            ...
            <aside className="fixed left-0 top-0 h-full w-[240px] bg-inverse-surface flex flex-col py-6 px-4 z-50">
                ...
                <div className="mt-auto border-t border-outline-variant/20 pt-6 flex flex-col gap-1">
                    <div className="flex items-center gap-3 px-4 py-2 text-secondary-fixed-dim">
                        ...
                        <span className="font-label-md text-label-md">{user?.name ?? 'Guest'}</span>
                    </div>
                    <div className="flex items-center gap-3 px-4 py-2 text-secondary-fixed-dim">
                        <span className="material-symbols-outlined">business</span>
                        <span className="font-label-md text-label-md">{user?.business?.name ?? '—'}</span>
                    </div>
                    <Link
                        href={route('logout')}
                        method="post"
                        as="button"
                        className="flex items-center gap-3 px-4 py-3 text-error/80 hover:text-error transition-colors w-full text-left"
                    >
