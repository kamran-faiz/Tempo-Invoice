  ...
  return (
    ...
    <div className="bg-surface-container-lowest w-full max-w-[480px] rounded-2xl shadow-2xl border border-outline-variant overflow-hidden transform transition-all duration-300 scale-100 animate-in fade-in zoom-in-95">
        <div className="flex justify-between items-center p-6 border-b border-outline-variant bg-surface-bright">
            ...
            <button className="w-10 h-10 flex items-center justify-center rounded-full text-on-surface-variant hover:bg-surface-container-high transition-colors" onclick="closeModal()">
            ...
            <div className="flex items-start gap-4 bg-error-container/10 p-4 rounded-xl border border-error/10 mb-2">
                ...
                <div>
                    <p className="font-body-md text-body-md text-on-surface leading-relaxed">
                        Invalid NTN format for B2B client. The provided NTN does not match the FBR database records. Please verify and resubmit.
                    </p>
                </div>
            </div>
            <p className="font-label-sm text-label-sm text-on-surface-variant mt-4 ml-1">
                Transmission ID: <span className="font-bold">TXN-99234812</span>
            </p>
        </div>
        <div className="flex justify-end p-6 border-t border-outline-variant bg-surface-container-low">
            <button className="bg-primary text-on-primary px-8 py-2.5 rounded-lg font-label-md text-label-md hover:opacity-90 active:scale-95 transition-all shadow-md" onclick="closeModal()">
                OK
            </button>
        </div>
    </div>
